  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-5">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <Metric label="Spent so far" value={formatNPR(total.spent)} tone="text-blue-700" />
        <Metric label="Completed value" value={formatNPR(total.completed)} tone="text-emerald-700" />
        <Metric label="Yet to complete" value={formatNPR(total.remaining)} tone="text-red-700" />
      </div>
      ...
      <div className="rounded-lg border border-[#ded6c8] bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#eee6d8] p-4">
          <div className="flex items-center gap-2 text-sm font-bold text-[#65706c]">
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
        <div className="rounded-lg border border-[#ded6c8] bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-[#eee6d8] px-5 py-3">
            <h2 className="text-sm font-black text-[#102a2b]">Budget lines behind this view</h2>
            <span className="text-xs font-bold text-[#65706c]">{visibleItems.length} shown</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-[#eee6d8] text-left text-[10px] font-black uppercase tracking-[0.14em] text-[#8c8272]"><th className="px-5 py-3">Project</th><th className="px-5 py-3">Area</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Allocated</th>{canPropose && <th className="px-5 py-3 text-right">Action</th>}</tr></thead>
              <tbody className="divide-y divide-[#f2ede4]">
                {visibleItems.length === 0 ? <tr><td colSpan={canPropose ? 5 : 4} className="px-5 py-12 text-center text-sm text-[#8c8272]"><Table2 className="mx-auto mb-2 h-7 w-7 text-[#cfc4b4]" />No budget lines in this level yet.</td></tr> : visibleItems.map(item => <tr key={item._id} className="hover:bg-[#fffaf2]"><td className="px-5 py-3"><p className="font-bold text-[#102a2b]">{item.title}</p><p className="text-xs text-[#65706c]">{item.department}</p></td><td className="px-5 py-3 text-xs text-[#65706c]">{item.district}{item.municipality ? `, ${item.municipality}` : ''}{item.ward ? `, Ward ${item.ward}` : ''}</td><td className="px-5 py-3"><StageBadge stage={item.status || 'planned'} /></td><td className="px-5 py-3 text-right font-black text-[#102a2b]">{formatNPR(item.amount)}</td>{canPropose && <td className="px-5 py-3 text-right"><button onClick={() => selectItem(item)} className="rounded-lg border border-[#ded6c8] px-3 py-1.5 text-xs font-black text-[#0f3d3e] hover:bg-[#eef6f4]">Edit</button></td>}</tr>)}
              </tbody>
            </table>
          </div>
        </div>
...
function Metric({ label, value, tone = 'text-[#102a2b]' }) {
  return <div className="rounded-lg border border-[#ded6c8] bg-white p-4 shadow-sm"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#8c8272]">{label}</p><p className={cn('mt-1 text-xl font-black tabular-nums', tone)}>{value}</p></div>;
}
function Tab({ active, onClick, icon: Icon, children }) {
  return <button onClick={onClick} className={cn('flex h-10 items-center gap-2 rounded-lg px-3 text-sm font-black', active ? 'bg-[#0f3d3e] text-white' : 'border border-[#ded6c8] text-[#65706c] hover:bg-[#f7f2ea]')}><Icon className="h-4 w-4" />{children}</button>;
}
function StageBadge({ stage }) {
  return <span className={cn('rounded-md px-2 py-1 text-[10px] font-black uppercase tracking-wide', STAGE_COLORS[stage] || STAGE_COLORS.planned)}>{stage}</span>;
}
function Progress({ node }) {
  return <div><div className="mb-1 flex justify-between text-xs font-bold text-[#65706c]"><span>{node.completion}% complete</span><span>{formatNPR(node.spent)} spent</span></div><div className="h-2 overflow-hidden rounded-full bg-[#eee6d8]"><div className="h-full rounded-full bg-[#0f3d3e]" style={{ width: `${Math.min(100, node.completion)}%` }} /></div></div>;
}
function MapView({ nodes, level, onDrill }) {
  return <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{nodes.map((n, i) => <button key={n.id} onClick={() => onDrill(n)} className="min-h-40 rounded-lg border border-[#ded6c8] bg-[#fffaf2] p-4 text-left transition hover:-translate-y-0.5 hover:border-[#0f3d3e] hover:shadow-md"><div className="flex items-start justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#dc143c]">{LEVEL_LABEL[level]}</p><h3 className="mt-1 text-lg font-black text-[#102a2b]">{n.name}</h3></div><span className="grid h-9 w-9 place-items-center rounded-lg bg-white text-sm font-black text-[#0f3d3e] shadow-sm">{i + 1}</span></div><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><p><span className="block text-[#8c8272]">Allocated</span><b>{formatNPR(n.allocated)}</b></p><p><span className="block text-[#8c8272]">Remaining</span><b>{formatNPR(n.remaining)}</b></p></div><div className="mt-4"><Progress node={n} /></div><div className="mt-3 flex flex-wrap gap-1.5"><StageBadge stage="planned" /><span className="text-[10px] font-bold text-[#65706c]">{n.projectCount} records</span></div></button>)}</div>;
}
function ListView({ nodes, level, onDrill }) {
  return <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-[#eee6d8] text-left text-[10px] font-black uppercase tracking-[0.14em] text-[#8c8272]"><th className="px-3 py-3">{LEVEL_LABEL[level]}</th><th className="px-3 py-3 text-right">Allocated</th><th className="px-3 py-3 text-right">Spent</th><th className="px-3 py-3">Completion</th><th className="px-3 py-3">Stages</th></tr></thead><tbody className="divide-y divide-[#f2ede4]">{nodes.map(n => <tr key={n.id} onClick={() => onDrill(n)} className="cursor-pointer hover:bg-[#fffaf2]"><td className="px-3 py-3 font-black text-[#102a2b]">{n.name}</td><td className="px-3 py-3 text-right font-bold">{formatNPR(n.allocated)}</td><td className="px-3 py-3 text-right font-bold text-blue-700">{formatNPR(n.spent)}</td><td className="min-w-56 px-3 py-3"><Progress node={n} /></td><td className="px-3 py-3 text-xs text-[#65706c]">{n.planned} planned / {n.ongoing} ongoing / {n.completedStage} done / {n.delayed} delayed</td></tr>)}</tbody></table></div>;
}
